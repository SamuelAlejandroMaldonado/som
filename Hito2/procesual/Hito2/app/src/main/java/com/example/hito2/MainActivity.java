    ...
    public void onClick(View v) {
            ...
            switch (v.getId())
            {
                ...
                case R.id.uscar:
                    ...
                    if(edt10.getText().toString().equals("confirmados") ||edt10.getText().toString().equals("CONFIRMADOS") )
                    {
                        if (n1 > n2) {
                            if (n1 > n3) {

                                Toast.makeText(this.getApplication(),"Cochabamba con:" +n1, Toast.LENGTH_LONG).show();

                            } else {

                                Toast.makeText(this.getApplication(),"Oruro: " +n3, Toast.LENGTH_LONG).show();

                            }
                        } else if (n2 > n3) {

                            Toast.makeText(this.getApplication(),"Santa Cruz: " +n2, Toast.LENGTH_LONG).show();
                        } else {

                            Toast.makeText(this.getApplication(),"Oruro: " +n3, Toast.LENGTH_LONG).show();
                        }
                    }
                   else if(edt10.getText().toString().equals("sospechosos") ||edt10.getText().toString().equals("SOSPECHOSOS") )
                    {
                        if (n4 > n5) {
                            if (n4 > n6) {

                                Toast.makeText(this.getApplication(),"cochabamba: " +n4, Toast.LENGTH_LONG).show();

                            } else {

                                Toast.makeText(this.getApplication(),"Oruro:  " +n6, Toast.LENGTH_LONG).show();

                            }
                        } else if (n5 > n6) {

                            Toast.makeText(this.getApplication(),"Santa Cruz: " +n5, Toast.LENGTH_LONG).show();
                        } else {
